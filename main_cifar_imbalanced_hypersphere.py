...
from cifar_imbalance import IMBALANCECIFAR10  # adjust path if needed
...
def train_one_epoch(model,centerloss, optimizer, optimizer_center, data_loader, device, epoch, args):
    ...
    for image, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        loss = 5 * intraclass_loss + 2.0 * triplet_loss + 5.0 * uniform_loss2 
...
def main(args):
    ...
    trainset = IMBALANCECIFAR10(
        ...
        imbalance_ratio=0.01,  # control how imbalanced it is
        root='./data',
        imb_type='exp'          # or 'step' or 'none'
    )

    testset = IMBALANCECIFAR10(
        train=False,
    ...
    data_loader_val = DataLoader(testset, batch_size=args.batch_size, shuffle=False, num_workers=4)
...
def parse_args():
    ...
    parser.add_argument('--Expand', default=16, type=int, metavar='N',
