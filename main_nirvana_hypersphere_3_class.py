...
def train_one_epoch(
    ...
    for image, target in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        total_loss = 5 * intraclass_loss + 2.0 * triplet_loss + 15.0 * uniform_loss2 
    ...
    ax.figure.savefig(os.path.join("figures_Hypersphere_3classes_u2_paper", "lambda_25_fig_%d.png" % epoch))
    plt.close("all")
...
def evaluate_model(model, centerloss, data_loader, device, epoch, args):
...
def main(args):
    ...
    print(args)
    centerloss = nirvana_hypersphere(args.num_classes, args.feat_dim, True, True, device, Expand=args.Expand)
    ...
    if args.test_only:
        return evaluate_model(model, centerloss, data_loader_val, device, epoch=0, args=args)

    print("Start training")
    start_time = time.time()
    for epoch in range(args.start_epoch, args.epochs):
        acc1, loss = train_one_epoch(
            model,
            centerloss,
            optimizer,
            optimizer_center,
        ...
        if epoch % args.eval_freq == 0 and epoch != 0:
            val_acc1, val_acc1_fc = evaluate_model(
...
def parse_args():
    ...
    parser.add_argument("--Expand", default=5, type=int, metavar="N", help="Expand factor of centers")
